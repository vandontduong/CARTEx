...
expt.obj@meta.data$characteristics_response <- plyr::mapvalues(x = expt.obj@meta.data$characteristics_response,
                                              from = c('Non-responder', 'Responder'),
                                              to = c('NR', 'R'))
expt.obj$characteristics_response <- factor(expt.obj$characteristics_response, levels = c("NR", "R"))
...
expt.obj@meta.data$Timepoint_Response <- paste(expt.obj@meta.data$Timepoint, expt.obj@meta.data$characteristics_response, sep = "-")
expt.obj@meta.data$Timepoint_Response <- factor(expt.obj@meta.data$Timepoint_Response, levels = c("Pre-NR","Pre-R","Post-NR","Post-R"))
...
barplot_response_seurat_clusters <- BarPlotStackSplit(expt.obj, 'characteristics_response', 'seurat_clusters', color_set = c("firebrick", "seagreen"))
...
barplot_therapy_seurat_clusters <- BarPlotStackSplit(expt.obj, 'characteristics_therapy', 'seurat_clusters', color_set = c("palevioletred", "cadetblue", "mediumpurple"))
generate_figs(barplot_therapy_seurat_clusters, paste('./plots/', experiment, '_prepare_barplot_therapy_seurat_clusters', sep = ''), c(8,4))

barplot_timepoint_seurat_clusters <- BarPlotStackSplit(expt.obj, 'Timepoint', 'seurat_clusters', color_set = c("lightblue", "steelblue"))
