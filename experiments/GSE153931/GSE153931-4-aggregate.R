...
Idents(query.obj.agg) <- "identifier4"
...
# query.obj.agg$identifier4 <- factor(query.obj.agg$identifier4, levels = c('Mild', 'Moderate', 'Severe', 'void', 'YoungNaive', 'OldTerminal'))
...
aggplot_CARTEx_84 <- md %>% ggplot(aes(identifier4, CARTEx_84)) +
  ...
  scale_fill_manual(values = c('cadetblue', 'steelblue', 'indianred', "royalblue", "orchid")) +
  ...
  ylab("CARTEx 84") + xlab(NULL) + geom_point() + ylim(-3, 3) + theme(legend.position="none")
...
aggplot_CARTEx_200 <- md %>% ggplot(aes(identifier4, CARTEx_200)) +
  geom_bar(stat = "summary", fun = "mean", aes(fill = identifier4)) +
  scale_fill_manual(values = c('cadetblue', 'steelblue', 'indianred', "royalblue", "orchid")) +
  stat_compare_means(method = "wilcox.test", comparisons = list(c('Mild','Moderate')), label = "p.signif", label.y = 2.4) +
  stat_compare_means(method = "wilcox.test", comparisons = list(c('Moderate','Severe')), label = "p.signif", label.y = 2.8) +
