...
expt.obj@meta.data$pblabels <- PseudoBulkLabels(expt.obj, 5)
expt.obj.agg <- AggregateExpression(expt.obj, group.by = c('AgeGroup2', 'Tex_linear_diff', 'pblabels'), return.seurat = TRUE)
...
aggplot_CARTEx_200_age_group_2_Tex_linear_diff_split <- md %>% ggplot(aes(x = AgeGroup2, y = CARTEx_200, color = Tex_linear_diff)) +
  geom_quasirandom(groupOnX = FALSE) + ylim(-2,2) +
  scale_color_manual(values = c('Tex-prog' = 'royalblue', 'Tex-term' = 'maroon', 'Unclassified' = 'lightgrey')) +
  theme_classic() + theme(axis.title.x = element_blank())
generate_figs(aggplot_CARTEx_200_age_group_2_Tex_linear_diff_split, paste('./plots/', experiment, '_aggplot_CARTEx_200_age_group_2_Tex_linear_diff_split', sep = ''), c(6,5)) 



# incorporate size
md_count <- expt.obj@meta.data %>% group_by(Tex_linear_diff, AgeGroup2, pblabels) %>% summarize(count = n(), .groups = 'drop')
md_count$pblabels <- as.character(md_count$pblabels)
...
md <- md %>% left_join(md_count, by = c("Tex_linear_diff", "AgeGroup2", "pblabels"))

aggplot_CARTEx_200_Tex_linear_diff_monaco_split_countsized <- md %>% ggplot(aes(x = AgeGroup2, y = CARTEx_200, color = Tex_linear_diff, size = count)) +
  geom_quasirandom(groupOnX = FALSE) + ylim(-2,2) +
  scale_color_manual(values = c('Tex-prog' = 'royalblue', 'Tex-term' = 'maroon', 'Unclassified' = 'lightgrey')) +
  theme_classic() + theme(text = element_text(size = 18), axis.title.x = element_blank()) + 
  scale_x_discrete(labels = c('N', 'U30', 'U50', 'U70', 'E')) + ylab('CARTEx') +
  scale_color_manual(labels=c("P", "T", "U"), values = c('royalblue', 'maroon', 'lightgrey'), name = 'Tex')
generate_figs(aggplot_CARTEx_200_Tex_linear_diff_monaco_split_countsized, paste('./plots/', experiment, '_aggplot_CARTEx_200_Tex_linear_diff_monaco_split_countsized', sep = ''), c(4.5,3)) 
