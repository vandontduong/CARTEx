...
generate_figs(ridgeplt, paste('./plots/', experiment, '_l_prepare_ridgeplt', sep = ''))
...
generate_figs(barplot_phase, paste('./plots/', experiment, '_l_prepare_barplot_phase', sep = ''))
...
generate_figs(umap_phase, paste('./plots/', experiment, '_l_prepare_umap_phase', sep = ''), c(3, 2))

umap_phase_highlight <- DimPlotHighlightIdents(expt.obj, Phase, 'umap', 'blue', 0.1, 3)
generate_figs(umap_phase_highlight, paste('./plots/', experiment, '_l_prepare_umap_phase_highlight', sep = ''), c(15, 6))


saveRDS(expt.obj, file = paste('./data/', experiment, '_l_cellcycle.rds', sep = ''))

# expt.obj <- readRDS(paste('./data/', experiment, '_l_cellcycle.rds', sep = ''))
...
saveRDS(azimuth.predictions, paste(file='./data/', experiment, '_l_prepare_azimuth_predictions.rds', sep = ''))
write.csv(azimuth.predictions, paste(file='./data/', experiment, '_l_prepare_azimuth_predictions.csv', sep = ''))
...
generate_figs(umap_predicted_azimuth, paste('./plots/', experiment, '_l_prepare_umap_predicted_azimuth', sep = ''))

umap_predicted_azimuth_highlight <- DimPlotHighlightIdents(expt.obj, azimuth, 'umap', 'blue', 0.1, 2)
generate_figs(umap_predicted_azimuth_highlight, paste('./plots/', experiment, '_l_prepare_umap_predicted_azimuth_highlight', sep = ''), c(12, 8))
...
saveRDS(monaco.predictions, file=paste('./data/', experiment, '_l_prepare_monaco_predictions.rds', sep = ''))
write.csv(monaco.predictions, file=paste('./data/', experiment, '_l_prepare_monaco_predictions.csv', sep = ''))
...
generate_figs(umap_predicted_monaco, paste('./plots/', experiment, '_l_prepare_umap_predicted_monaco', sep = ''), c(2.9, 2))
...
saveRDS(dice.predictions, file=paste('./data/', experiment, '_l_prepare_dice_predictions.rds', sep = ''))
write.csv(dice.predictions, file=paste('./data/', experiment, '_l_prepare_dice_predictions.csv', sep = ''))
...
generate_figs(umap_predicted_dice, paste('./plots/', experiment, '_l_prepare_umap_predicted_dice', sep = ''))

umap_predicted_dice_highlight <- DimPlotHighlightIdents(expt.obj, dice, 'umap', 'blue', 0.1, 2)
generate_figs(umap_predicted_dice_highlight, paste('./plots/', experiment, '_l_prepare_umap_predicted_dice_highlight', sep = ''), c(12, 8))


featureplot_Tcell_markers <- FeaturePlot(expt.obj, features = c("CD8A", "CD8B", "PDCD1"))
generate_figs(featureplot_Tcell_markers, paste('./plots/', experiment, '_l_prepare_featureplot_Tcell_markers', sep = ''))
...
generate_figs(barplot_monaco_seurat_clusters, paste('./plots/', experiment, '_l_prepare_barplot_monaco_seurat_clusters', sep = ''), c(8,4))

barplot_dice_seurat_clusters <- BarPlotStackSplit(expt.obj, 'dice', 'seurat_clusters')
generate_figs(barplot_dice_seurat_clusters, paste('./plots/', experiment, '_l_prepare_barplot_dice_seurat_clusters', sep = ''), c(8,4))

barplot_phase_seurat_clusters <- BarPlotStackSplit(expt.obj, 'Phase', 'seurat_clusters')
generate_figs(barplot_phase_seurat_clusters, paste('./plots/', experiment, '_l_prepare_barplot_phase_seurat_clusters', sep = ''), c(8,4))

barplot_azimuth_age_group <- BarPlotStackSplit(expt.obj, 'azimuth', 'AgeGroup2')
generate_figs(barplot_azimuth_age_group, paste('./plots/', experiment, '_l_prepare_barplot_azimuth_age_group', sep = ''), c(8,4))

barplot_monaco_age_group <- BarPlotStackSplit(expt.obj, 'monaco', 'AgeGroup2', color_set = c('deepskyblue','seagreen','darkgoldenrod','plum3')) + scale_x_discrete(labels = c("U50", "U70", "E"))
generate_figs(barplot_monaco_age_group, paste('./plots/', experiment, '_l_prepare_barplot_monaco_age_group', sep = ''), c(4,2))

barplot_dice_age_group <- BarPlotStackSplit(expt.obj, 'dice', 'AgeGroup2')
generate_figs(barplot_dice_age_group, paste('./plots/', experiment, '_l_prepare_barplot_dice_age_group', sep = ''), c(8,4))

barplot_phase_age_group <- BarPlotStackSplit(expt.obj, 'Phase', 'AgeGroup2', color_set = hcl.colors(3, palette = "Temps")) + scale_x_discrete(labels = c("U50", "U70", "E"))
generate_figs(barplot_phase_age_group, paste('./plots/', experiment, '_l_prepare_barplot_phase_age_group', sep = ''), c(8,4))


barplot_monaco_age_group_slim <- barplot_monaco_age_group + theme(legend.position = "none")
generate_figs(barplot_monaco_age_group_slim, paste('./plots/', experiment, '_l_prepare_barplot_monaco_age_group_slim', sep = ''), c(1.5,1.75))

barplot_phase_age_group_slim <- barplot_phase_age_group + theme(legend.position = "none")
generate_figs(barplot_phase_age_group_slim, paste('./plots/', experiment, '_l_prepare_barplot_phase_age_group_slim', sep = ''), c(1.5,1.75))

###
###
###

saveRDS(expt.obj, file = paste('./data/', experiment, '_l_annotated.rds', sep = ''))

# expt.obj <- readRDS(paste('./data/', experiment, '_l_annotated.rds', sep = ''))
...
# generate_figs(rogue_boxplot_seurat_clusters_group, paste('./plots/', experiment, '_l_prepare_rogue_boxplot_seurat_clusters_group', sep = ''), c(8,4))

# rogue.res <- EntropyScore(expt.obj, 'monaco', 'Group')
# rogue_boxplot_monaco_group <- rogue.boxplot(rogue.res)
# generate_figs(rogue_boxplot_monaco_group, paste('./plots/', experiment, '_l_prepare_rogue_boxplot_monaco_group', sep = ''), c(8,4))
...
generate_figs(umap_sig_activationi_highlight, paste('./plots/', experiment, '_l_prepare_umap_sig_activationi_highlight', sep = ''), c(10, 8))

umap_sig_anergyi <- DimPlot(expt.obj, group.by = "Anergyi", shuffle = TRUE, seed = 123)
generate_figs(umap_sig_anergyi, paste('./plots/', experiment, '_l_prepare_umap_sig_anergyi', sep = ''))

umap_sig_anergyi_highlight <- DimPlotHighlightIdents(expt.obj, Anergyi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_anergyi_highlight, paste('./plots/', experiment, '_l_prepare_umap_sig_anergyi_highlight', sep = ''), c(10, 8))

umap_sig_stemnessi <- DimPlot(expt.obj, group.by = "Stemnessi", shuffle = TRUE, seed = 123)
generate_figs(umap_sig_stemnessi, paste('./plots/', experiment, '_l_prepare_umap_sig_stemnessi', sep = ''))

umap_sig_stemnessi_highlight <- DimPlotHighlightIdents(expt.obj, Stemnessi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_stemnessi_highlight, paste('./plots/', experiment, '_l_prepare_umap_sig_stemnessi_highlight', sep = ''), c(10, 8))

umap_sig_senescencei <- DimPlot(expt.obj, group.by = "Senescencei", shuffle = TRUE, seed = 123)
generate_figs(umap_sig_senescencei, paste('./plots/', experiment, '_l_prepare_umap_sig_senescencei', sep = ''))

umap_sig_senescencei_highlight <- DimPlotHighlightIdents(expt.obj, Stemnessi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_senescencei_highlight, paste('./plots/', experiment, '_l_prepare_umap_sig_senescencei_highlight', sep = ''), c(10, 8))
...
generate_figs(umap_CARTEx_84, paste('./plots/', experiment, '_l_prepare_umap_CARTEx_84', sep = ''), c(2.8, 2))
generate_figs(umap_CARTEx_200, paste('./plots/', experiment, '_l_prepare_umap_CARTEx_200', sep = ''), c(2.8, 2))
generate_figs(umap_CARTEx_630, paste('./plots/', experiment, '_l_prepare_umap_CARTEx_630', sep = ''), c(2.8, 2))
generate_figs(umap_sig_activation, paste('./plots/', experiment, '_l_prepare_umap_sig_activation', sep = ''), c(2.8, 2))
generate_figs(umap_sig_anergy, paste('./plots/', experiment, '_l_prepare_umap_sig_anergy', sep = ''), c(2.8, 2))
generate_figs(umap_sig_stemness, paste('./plots/', experiment, '_l_prepare_umap_sig_stemness', sep = ''), c(2.8, 2))
generate_figs(umap_sig_senescence, paste('./plots/', experiment, '_l_prepare_umap_sig_senescence', sep = ''), c(2.8, 2))
generate_figs(umap_NKlike_Tex, paste('./plots/', experiment, '_l_prepare_umap_NKlike_Tex', sep = ''), c(2.8, 2))
generate_figs(umap_LCMV_Tex, paste('./plots/', experiment, '_l_prepare_umap_LCMV_Tex', sep = ''), c(2.8, 2))
generate_figs(umap_BBD_Tex, paste('./plots/', experiment, '_l_prepare_umap_BBD_Tex', sep = ''), c(2.8, 2))
generate_figs(umap_PD1_Tex, paste('./plots/', experiment, '_l_prepare_umap_PD1_Tex', sep = ''), c(2.8, 2))
generate_figs(umap_TSR, paste('./plots/', experiment, '_l_prepare_umap_TSR', sep = ''), c(2.8, 2))
generate_figs(umap_Tex_Term, paste('./plots/', experiment, '_l_prepare_umap_Tex_Term', sep = ''), c(2.8, 2))
generate_figs(umap_Tex_KLR, paste('./plots/', experiment, '_l_prepare_umap_Tex_KLR', sep = ''), c(2.8, 2))
