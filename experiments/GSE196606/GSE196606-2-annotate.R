...
### Script name: GSE196606-2-annotate.R
### Description: annotate seurat object for GSE196606
### Author: Vandon Duong
...
barplot_monaco_group_slim <- barplot_monaco_group + theme(legend.position = "none") + scale_x_discrete(labels = c('C', 'L', 'N'))
generate_figs(barplot_monaco_group_slim, paste('./plots/', experiment, '_prepare_barplot_monaco_group_slim', sep = ''), c(1.25,1.75))

barplot_phase_group_slim <- barplot_phase_group + theme(legend.position = "none") + scale_x_discrete(labels = c('C', 'L', 'N'))
generate_figs(barplot_phase_group_slim, paste('./plots/', experiment, '_prepare_barplot_phase_group_slim', sep = ''), c(1.25,1.75))
...
vlnplot_group_exhaustion_markers <- VlnPlot(expt.obj, features = c('PDCD1', 'HAVCR2', 'LAG3', 'CTLA4', 'NT5E', 'ENTPD1'), group.by = 'group', ncol = 3, cols = c('red', 'green', 'blue'), y.max = 4)
