### Script name: GSE136874-3-integrate.R
...
expt.obj@meta.data[['identifier2']] <- expt.obj@meta.data$CAR
...
query.obj$identifier2 <- factor(query.obj$identifier2, levels = c("CD19", "GD2", "Newborn", "YoungNaive", "OldTerminal"))
vlnplot_CARTEx_84 <- VlnPlot(query.obj, features = c("CARTEx_84"), group.by = 'identifier2', y.max = 6, pt.size = 0) + 
  ...
  stat_compare_means(method = "wilcox.test", comparisons = list(c('YoungNaive','GD2')), label = "p.signif", label.y = 4.5) +
  stat_compare_means(method = "wilcox.test", comparisons = list(c('CD19','YoungNaive')), label = "p.signif", label.y = 5.5) +
  stat_compare_means(method = "wilcox.test", comparisons = list(c('GD2', 'OldTerminal')), label = "p.signif", label.y = 3.5)
