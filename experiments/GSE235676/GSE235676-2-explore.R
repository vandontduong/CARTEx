...
CART.combined.CD8pos <- SetIdent(CART.combined.CD8pos, value = "age")
...
umap_age <- DimPlot(CART.combined.CD8pos, reduction = "umap", group.by = "age", shuffle = TRUE, seed = 123)
generate_figs(umap_age, paste('./plots/', experiment, '_umap_age', sep = ''))

umap_gender <- DimPlot(CART.combined.CD8pos, reduction = "umap", group.by = "gender", shuffle = TRUE, seed = 123)
generate_figs(umap_gender, paste('./plots/', experiment, '_umap_gender', sep = ''))

umap_pt <- DimPlot(CART.combined.CD8pos, reduction = "umap", group.by = "Pt_number", shuffle = TRUE, seed = 123)
generate_figs(umap_pt, paste('./plots/', experiment, '_umap_pt', sep = ''))

umap_diagnosis <- DimPlot(CART.combined.CD8pos, reduction = "umap", group.by = "diagnosis", shuffle = TRUE, seed = 123)
generate_figs(umap_diagnosis, paste('./plots/', experiment, '_umap_diagnosis', sep = ''))

umap_treatment_1 <- DimPlot(CART.combined.CD8pos, reduction = "umap", group.by = "treatment_1", shuffle = TRUE, seed = 123)
generate_figs(umap_treatment_1, paste('./plots/', experiment, '_umap_treatment_1', sep = ''))

umap_treatment_3 <- DimPlot(CART.combined.CD8pos, reduction = "umap", group.by = "treatment_3", shuffle = TRUE, seed = 123)
generate_figs(umap_treatment_3, paste('./plots/', experiment, '_umap_treatment_3', sep = ''))

umap_PD1_response <- DimPlot(CART.combined.CD8pos, reduction = "umap", group.by = "PD1", shuffle = TRUE, seed = 123)
generate_figs(umap_PD1_response, paste('./plots/', experiment, '_umap_PD1_response', sep = ''))


# ANALYZE CELL CYCLE DATA FROM SEURAT OBJECT
md <- CART.combined.CD8pos@meta.data %>% as.data.table
md[, .N, by = c("Pt_number", "treatment_1", "treatment_3")]
md[, .N, by = c("Pt_number", "Phase")]
