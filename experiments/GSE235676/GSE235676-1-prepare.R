...
CART.combined <- readRDS(paste('./data/GSE235676_GBM.RNA.integrated.24.rds', sep = ''))
CART.combined <- SetIdent(CART.combined, value = "anno_ident")
CART.combined <- subset(CART.combined, idents = c("T cells"))
...
# Examine features
vlnplot_quality <- VlnPlot(object = CART.combined.CD8pos, features = c('nFeature_RNA','nCount_RNA', "percent.mt"), group.by = 'anno_ident', ncol=3)
