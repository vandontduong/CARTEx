...
### Script name: Zenodo3993994-csf-2-annotate.R
...
expt.obj <- readRDS(paste('./data/', experiment, '_csf.rds', sep = ''))
...
generate_figs(ridgeplt, paste('./plots/', experiment, '_csf_prepare_ridgeplt', sep = ''))

expt.obj <- RunPCA(expt.obj, features = c(s.genes, g2m.genes))
PCAplot <- DimPlot(expt.obj, reduction = 'pca')
generate_figs(PCAplot, paste('./plots/', experiment, '_csf_prepare_PCAplot', sep = ''), c(6, 5))
...
generate_figs(PCAplot_regressed, paste('./plots/', experiment, '_csf_prepare_PCAplot_regressed', sep = ''), c(6, 5))

expt.obj@meta.data$Phase <- factor(expt.obj@meta.data$Phase, levels = c('G1', 'S', 'G2M'))

# TEMP
saveRDS(expt.obj, file = paste('./data/', experiment, '_csf_cellcycle.rds', sep = ''))
...
generate_figs(barplot_phase, paste('./plots/', experiment, '_csf_prepare_barplot_phase', sep = ''))

umap_phase <- DimPlot(expt.obj, reduction = 'umap', group.by = "Phase", cols = phase_data$cols, shuffle = TRUE, seed = 123)
generate_figs(umap_phase, paste('./plots/', experiment, '_csf_prepare_umap_phase', sep = ''), c(6, 5))

umap_phase_highlight <- DimPlotHighlightIdents(expt.obj, Phase, 'umap', 'blue', 0.1, 3)
generate_figs(umap_phase_highlight, paste('./plots/', experiment, '_csf_prepare_umap_phase_highlight', sep = ''), c(15, 6))


# saveRDS(expt.obj, file = paste('./data/', experiment, '_csf_cellcycle.rds', sep = ''))

# expt.obj <- readRDS(paste('./data/', experiment, '_csf_cellcycle.rds', sep = ''))
...
saveRDS(azimuth.predictions, paste(file='./data/', experiment, '_csf_azimuth_predictions.rds', sep = ''))
write.csv(azimuth.predictions, paste(file='./data/', experiment, '_csf_azimuth_predictions.csv', sep = ''))
...
generate_figs(umap_predicted_azimuth, paste('./plots/', experiment, '_csf_umap_predicted_azimuth', sep = ''), c(6.5, 5))

umap_predicted_azimuth_highlight <- DimPlotHighlightIdents(expt.obj, azimuth, 'umap', 'blue', 0.1, 2)
generate_figs(umap_predicted_azimuth_highlight, paste('./plots/', experiment, '_csf_prepare_umap_predicted_azimuth_highlight', sep = ''), c(12, 8))
...
saveRDS(monaco.predictions, file=paste('./data/', experiment, '_csf_monaco_predictions.rds', sep = ''))
write.csv(monaco.predictions, file=paste('./data/', experiment, '_csf_monaco_predictions.csv', sep = ''))
...
generate_figs(umap_predicted_monaco, paste('./plots/', experiment, '_csf_prepare_umap_predicted_monaco', sep = ''), c(7.5, 5))

umap_predicted_monaco_highlight <- DimPlotHighlightIdents(expt.obj, monaco, 'umap', 'blue', 0.1, 2)
generate_figs(umap_predicted_monaco_highlight, paste('./plots/', experiment, '_csf_prepare_umap_predicted_monaco_highlight', sep = ''), c(22, 20))
...
saveRDS(dice.predictions, file=paste('./data/', experiment, '_csf_dice_predictions.rds', sep = ''))
write.csv(dice.predictions, file=paste('./data/', experiment, '_csf_dice_predictions.csv', sep = ''))
...
generate_figs(umap_predicted_dice, paste('./plots/', experiment, '_csf_umap_predicted_dice', sep = ''), c(6.5, 5))

umap_predicted_dice_highlight <- DimPlotHighlightIdents(expt.obj, dice, 'umap', 'blue', 0.1, 2)
generate_figs(umap_predicted_dice_highlight, paste('./plots/', experiment, '_csf_prepare_umap_predicted_dice_highlight', sep = ''), c(12, 8))


featureplot_Tcell_markers <- FeaturePlot(expt.obj, features = c("CD8A", "CD8B", "PDCD1"))
generate_figs(featureplot_Tcell_markers, paste('./plots/', experiment, '_csf_prepare_featureplot_Tcell_markers', sep = ''))
...
generate_figs(barplot_azimuth_seurat_clusters, paste('./plots/', experiment, '_csf_prepare_barplot_azimuth_seurat_clusters', sep = ''), c(8,4))

barplot_monaco_seurat_clusters <- BarPlotStackSplit(expt.obj, 'monaco', 'seurat_clusters')
generate_figs(barplot_monaco_seurat_clusters, paste('./plots/', experiment, '_csf_prepare_barplot_monaco_seurat_clusters', sep = ''), c(8,4))

barplot_dice_seurat_clusters <- BarPlotStackSplit(expt.obj, 'dice', 'seurat_clusters')
generate_figs(barplot_dice_seurat_clusters, paste('./plots/', experiment, '_csf_prepare_barplot_dice_seurat_clusters', sep = ''), c(8,4))

barplot_azimuth_disease <- BarPlotStackSplit(expt.obj, 'azimuth', 'Disease')
generate_figs(barplot_azimuth_disease, paste('./plots/', experiment, '_csf_prepare_barplot_azimuth_disease', sep = ''), c(8,4))

barplot_monaco_disease <- BarPlotStackSplit(expt.obj, 'monaco', 'Disease', color_set = c('deepskyblue', 'seagreen', 'darkgoldenrod', 'plum3'))
generate_figs(barplot_monaco_disease, paste('./plots/', experiment, '_csf_prepare_barplot_monaco_disease', sep = ''), c(6,4))

barplot_dice_disease <- BarPlotStackSplit(expt.obj, 'dice', 'Disease')
generate_figs(barplot_dice_disease, paste('./plots/', experiment, '_csf_prepare_barplot_dice_disease', sep = ''), c(8,4))

barplot_phase_disease <- BarPlotStackSplit(expt.obj, 'Phase', 'Disease', color_set = hcl.colors(3, palette = "Temps"))
generate_figs(barplot_phase_disease, paste('./plots/', experiment, '_csf_prepare_barplot_phase_disease', sep = ''), c(5,4))


saveRDS(expt.obj, file = paste('./data/', experiment, '_csf_annotated.rds', sep = ''))

# expt.obj <- readRDS(paste('./data/', experiment, '_csf_annotated.rds', sep = ''))
...
generate_figs(umap_sig_activationi, paste('./plots/', experiment, '_csf_prepare_umap_sig_activationi', sep = ''), c(6,5))

umap_sig_activationi_highlight <- DimPlotHighlightIdents(expt.obj, Activationi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_activationi_highlight, paste('./plots/', experiment, '_csf_prepare_umap_sig_activationi_highlight', sep = ''), c(10, 8))

umap_sig_anergyi <- DimPlot(expt.obj, group.by = "Anergyi", shuffle = TRUE, seed = 123)
generate_figs(umap_sig_anergyi, paste('./plots/', experiment, '_csf_prepare_umap_sig_anergyi', sep = ''), c(6,5))

umap_sig_anergyi_highlight <- DimPlotHighlightIdents(expt.obj, Anergyi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_anergyi_highlight, paste('./plots/', experiment, '_csf_prepare_umap_sig_anergyi_highlight', sep = ''), c(10, 8))

umap_sig_stemnessi <- DimPlot(expt.obj, group.by = "Stemnessi", shuffle = TRUE, seed = 123)
generate_figs(umap_sig_stemnessi, paste('./plots/', experiment, '_csf_prepare_umap_sig_stemnessi', sep = ''), c(6,5))

umap_sig_stemnessi_highlight <- DimPlotHighlightIdents(expt.obj, Stemnessi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_stemnessi_highlight, paste('./plots/', experiment, '_csf_prepare_umap_sig_stemnessi_highlight', sep = ''), c(10, 8))

umap_sig_senescencei <- DimPlot(expt.obj, group.by = "Senescencei", shuffle = TRUE, seed = 123)
generate_figs(umap_sig_senescencei, paste('./plots/', experiment, '_csf_prepare_umap_sig_senescencei', sep = ''), c(6,5))

umap_sig_senescencei_highlight <- DimPlotHighlightIdents(expt.obj, Stemnessi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_senescencei_highlight, paste('./plots/', experiment, '_csf_prepare_umap_sig_senescencei_highlight', sep = ''), c(10, 8))
...
generate_figs(umap_CARTEx_84, paste('./plots/', experiment, '_csf_prepare_umap_CARTEx_84', sep = ''), c(2.8, 2))
generate_figs(umap_CARTEx_200, paste('./plots/', experiment, '_csf_prepare_umap_CARTEx_200', sep = ''), c(2.8, 2))
generate_figs(umap_CARTEx_630, paste('./plots/', experiment, '_csf_prepare_umap_CARTEx_630', sep = ''), c(2.8, 2))
generate_figs(umap_sig_activation, paste('./plots/', experiment, '_csf_prepare_umap_sig_activation', sep = ''), c(2.8, 2))
generate_figs(umap_sig_anergy, paste('./plots/', experiment, '_csf_prepare_umap_sig_anergy', sep = ''), c(2.8, 2))
generate_figs(umap_sig_stemness, paste('./plots/', experiment, '_csf_prepare_umap_sig_stemness', sep = ''), c(2.8, 2))
generate_figs(umap_sig_senescence, paste('./plots/', experiment, '_csf_prepare_umap_sig_senescence', sep = ''), c(2.8, 2))
generate_figs(umap_NKlike_Tex, paste('./plots/', experiment, '_csf_prepare_umap_NKlike_Tex', sep = ''), c(2.8, 2))
generate_figs(umap_LCMV_Tex, paste('./plots/', experiment, '_csf_prepare_umap_LCMV_Tex', sep = ''), c(2.8, 2))
generate_figs(umap_BBD_Tex, paste('./plots/', experiment, '_csf_prepare_umap_BBD_Tex', sep = ''), c(2.8, 2))
generate_figs(umap_PD1_Tex, paste('./plots/', experiment, '_csf_prepare_umap_PD1_Tex', sep = ''), c(2.8, 2))
...
generate_figs(dmap_CARTEx_84, paste('./plots/', experiment, '_csf_prepare_dmap_CARTEx_84', sep = ''), c(2.8, 2))
generate_figs(dmap_CARTEx_200, paste('./plots/', experiment, '_csf_prepare_dmap_CARTEx_200', sep = ''), c(2.8, 2))
generate_figs(dmap_CARTEx_630, paste('./plots/', experiment, '_csf_prepare_dmap_CARTEx_630', sep = ''), c(2.8, 2))
generate_figs(dmap_sig_activation, paste('./plots/', experiment, '_csf_prepare_dmap_sig_activation', sep = ''), c(2.8, 2))
generate_figs(dmap_sig_anergy, paste('./plots/', experiment, '_csf_prepare_dmap_sig_anergy', sep = ''), c(2.8, 2))
generate_figs(dmap_sig_stemness, paste('./plots/', experiment, '_csf_prepare_dmap_sig_stemness', sep = ''), c(2.8, 2))
generate_figs(dmap_sig_senescence, paste('./plots/', experiment, '_csf_prepare_dmap_sig_senescence', sep = ''), c(2.8, 2))


saveRDS(expt.obj, file = paste('./data/', experiment, '_csf_scored.rds', sep = ''))
