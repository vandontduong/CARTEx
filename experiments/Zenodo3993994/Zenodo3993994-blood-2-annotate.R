...
### Script name: Zenodo3993994-blood-2-annotate.R
...
expt.obj <- readRDS(paste('./data/', experiment, '_blood.rds', sep = ''))
...
generate_figs(ridgeplt, paste('./plots/', experiment, '_blood_prepare_ridgeplt', sep = ''))

expt.obj <- RunPCA(expt.obj, features = c(s.genes, g2m.genes))
PCAplot <- DimPlot(expt.obj, reduction = 'pca')
generate_figs(PCAplot, paste('./plots/', experiment, '_blood_prepare_PCAplot', sep = ''), c(6, 5))
...
generate_figs(PCAplot_regressed, paste('./plots/', experiment, '_blood_prepare_PCAplot_regressed', sep = ''), c(6, 5))

expt.obj@meta.data$Phase <- factor(expt.obj@meta.data$Phase, levels = c('G1', 'S', 'G2M'))

# TEMP
saveRDS(expt.obj, file = paste('./data/', experiment, '_blood_cellcycle.rds', sep = ''))
...
# expt.obj <- readRDS(paste('./data/', experiment, '_blood_cellcycle.rds', sep = ''))
...
saveRDS(azimuth.predictions, paste(file='./data/', experiment, '_blood_azimuth_predictions.rds', sep = ''))
write.csv(azimuth.predictions, paste(file='./data/', experiment, '_blood_azimuth_predictions.csv', sep = ''))
...
generate_figs(umap_predicted_azimuth, paste('./plots/', experiment, '_blood_umap_predicted_azimuth', sep = ''), c(6.5, 5))

umap_predicted_azimuth_highlight <- DimPlotHighlightIdents(expt.obj, azimuth, 'umap', 'blue', 0.1, 2)
generate_figs(umap_predicted_azimuth_highlight, paste('./plots/', experiment, '_blood_prepare_umap_predicted_azimuth_highlight', sep = ''), c(12, 8))
...
saveRDS(monaco.predictions, file=paste('./data/', experiment, '_blood_monaco_predictions.rds', sep = ''))
write.csv(monaco.predictions, file=paste('./data/', experiment, '_blood_monaco_predictions.csv', sep = ''))
...
saveRDS(dice.predictions, file=paste('./data/', experiment, '_blood_dice_predictions.rds', sep = ''))
write.csv(dice.predictions, file=paste('./data/', experiment, '_blood_dice_predictions.csv', sep = ''))
...
generate_figs(umap_predicted_dice, paste('./plots/', experiment, '_blood_umap_predicted_dice', sep = ''), c(6.5, 5))

umap_predicted_dice_highlight <- DimPlotHighlightIdents(expt.obj, dice, 'umap', 'blue', 0.1, 2)
generate_figs(umap_predicted_dice_highlight, paste('./plots/', experiment, '_blood_prepare_umap_predicted_dice_highlight', sep = ''), c(12, 8))


featureplot_Tcell_markers <- FeaturePlot(expt.obj, features = c("CD8A", "CD8B", "PDCD1"))
generate_figs(featureplot_Tcell_markers, paste('./plots/', experiment, '_blood_prepare_featureplot_Tcell_markers', sep = ''))
...
generate_figs(barplot_azimuth_seurat_clusters, paste('./plots/', experiment, '_blood_prepare_barplot_azimuth_seurat_clusters', sep = ''), c(8,4))

barplot_monaco_seurat_clusters <- BarPlotStackSplit(expt.obj, 'monaco', 'seurat_clusters')
generate_figs(barplot_monaco_seurat_clusters, paste('./plots/', experiment, '_blood_prepare_barplot_monaco_seurat_clusters', sep = ''), c(8,4))

barplot_dice_seurat_clusters <- BarPlotStackSplit(expt.obj, 'dice', 'seurat_clusters')
generate_figs(barplot_dice_seurat_clusters, paste('./plots/', experiment, '_blood_prepare_barplot_dice_seurat_clusters', sep = ''), c(8,4))

barplot_azimuth_disease <- BarPlotStackSplit(expt.obj, 'azimuth', 'Disease')
generate_figs(barplot_azimuth_disease, paste('./plots/', experiment, '_blood_prepare_barplot_azimuth_disease', sep = ''), c(8,4))

barplot_monaco_disease <- BarPlotStackSplit(expt.obj, 'monaco', 'Disease', color_set = c('deepskyblue', 'seagreen', 'darkgoldenrod', 'plum3'))
generate_figs(barplot_monaco_disease, paste('./plots/', experiment, '_blood_prepare_barplot_monaco_disease', sep = ''), c(6,4))

barplot_dice_disease <- BarPlotStackSplit(expt.obj, 'dice', 'Disease')
generate_figs(barplot_dice_disease, paste('./plots/', experiment, '_blood_prepare_barplot_dice_disease', sep = ''), c(8,4))

barplot_phase_disease <- BarPlotStackSplit(expt.obj, 'Phase', 'Disease', color_set = hcl.colors(3, palette = "Temps"))
generate_figs(barplot_phase_disease, paste('./plots/', experiment, '_blood_prepare_barplot_phase_disease', sep = ''), c(5,4))

custom_labels <- c('Parkinson', 'Control')
barplot_phase_sample_type <- BarPlotStackSplit(expt.obj, 'Phase', 'sampleType', color_set = hcl.colors(3, palette = "Temps")) + 
  scale_x_discrete(labels = custom_labels) + theme_classic() + theme(axis.title.x = element_blank()) + labs(y = "Fraction")
generate_figs(barplot_phase_sample_type, paste('./plots/', experiment, '_blood_prepare_barplot_phase_sample_type', sep = ''), c(5,4))

barplot_monaco_sample_type <- BarPlotStackSplit(expt.obj, 'monaco', 'sampleType', color_set = c('deepskyblue', 'seagreen', 'darkgoldenrod', 'plum3')) + 
  scale_x_discrete(labels = custom_labels) + theme_classic() + theme(axis.title.x = element_blank()) + labs(y = "Fraction")
...
# expt.obj <- readRDS(paste('./data/', experiment, '_blood_annotated.rds', sep = ''))
...
generate_figs(umap_sig_activationi, paste('./plots/', experiment, '_blood_prepare_umap_sig_activationi', sep = ''), c(6,5))

umap_sig_activationi_highlight <- DimPlotHighlightIdents(expt.obj, Activationi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_activationi_highlight, paste('./plots/', experiment, '_blood_prepare_umap_sig_activationi_highlight', sep = ''), c(10, 8))

umap_sig_anergyi <- DimPlot(expt.obj, group.by = "Anergyi", shuffle = TRUE, seed = 123)
generate_figs(umap_sig_anergyi, paste('./plots/', experiment, '_blood_prepare_umap_sig_anergyi', sep = ''), c(6,5))

umap_sig_anergyi_highlight <- DimPlotHighlightIdents(expt.obj, Anergyi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_anergyi_highlight, paste('./plots/', experiment, '_blood_prepare_umap_sig_anergyi_highlight', sep = ''), c(10, 8))

umap_sig_stemnessi <- DimPlot(expt.obj, group.by = "Stemnessi", shuffle = TRUE, seed = 123)
generate_figs(umap_sig_stemnessi, paste('./plots/', experiment, '_blood_prepare_umap_sig_stemnessi', sep = ''), c(6,5))

umap_sig_stemnessi_highlight <- DimPlotHighlightIdents(expt.obj, Stemnessi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_stemnessi_highlight, paste('./plots/', experiment, '_blood_prepare_umap_sig_stemnessi_highlight', sep = ''), c(10, 8))

umap_sig_senescencei <- DimPlot(expt.obj, group.by = "Senescencei", shuffle = TRUE, seed = 123)
generate_figs(umap_sig_senescencei, paste('./plots/', experiment, '_blood_prepare_umap_sig_senescencei', sep = ''), c(6,5))

umap_sig_senescencei_highlight <- DimPlotHighlightIdents(expt.obj, Stemnessi, 'umap', 'blue', 0.1, 4)
generate_figs(umap_sig_senescencei_highlight, paste('./plots/', experiment, '_blood_prepare_umap_sig_senescencei_highlight', sep = ''), c(10, 8))
...
dmap_CARTEx_200 <- FeaturePlot(expt.obj, reduction = 'dm', features = c("CARTEx_200"), order = FALSE, pt.size = 0.1) + fix.sc + theme(plot.title = element_blank()) + xlim(c(-0.15, 0.15)) + ylim(c(-0.15, 0.15))
dmap_CARTEx_630 <- FeaturePlot(expt.obj, reduction = 'dm', features = c("CARTEx_630"), order = FALSE, pt.size = 0.1) + fix.sc + theme(plot.title = element_blank()) + xlim(c(-0.15, 0.15)) + ylim(c(-0.15, 0.15))
dmap_sig_activation <- FeaturePlot(expt.obj, reduction = 'dm', features = c("Activation"), order = FALSE, pt.size = 0.1) + fix.sc + theme(plot.title = element_blank()) + xlim(c(-0.15, 0.15)) + ylim(c(-0.15, 0.15))
dmap_sig_anergy <- FeaturePlot(expt.obj, reduction = 'dm', features = c("Anergy"), order = FALSE, pt.size = 0.1) + fix.sc + theme(plot.title = element_blank()) + xlim(c(-0.15, 0.15)) + ylim(c(-0.15, 0.15))
...
dmap_monaco <- DimPlot(expt.obj, reduction = "dm", group.by = "monaco", shuffle = TRUE, seed = 123, cols = c('deepskyblue', 'seagreen', 'darkgoldenrod', 'plum3'), pt.size = 0.1) + xlim(c(-0.15, 0.15)) + ylim(c(-0.15, 0.15)) + 
  theme(plot.title = element_blank()) + scale_color_manual(labels=c("N", "CM", "EM", "TE"), values = c('deepskyblue', 'seagreen', 'darkgoldenrod', 'plum3'))

generate_figs(dmap_monaco, paste('./plots/', experiment, '_blood_prepare_dmap_monaco', sep = ''), c(2.8, 2))
