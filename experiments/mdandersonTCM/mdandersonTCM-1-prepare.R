...
### Script name: mdandersonTCM-1-prepare.R
### Description: prepare seurat object for mdandersonTCM
### Author: Vandon Duong
...
expt.obj <- readRDS(paste('./data/CD8.rds', sep = ''))

expt.obj <- NormalizeData(expt.obj)


expt.obj@meta.data$nFeature_RNA <- colSums(x = GetAssayData(object = expt.obj[['RNA']], slot = "counts") > 0)
expt.obj@meta.data$nCount_RNA <- colSums(x = expt.obj[['RNA']], slot = "counts")
...
expt.obj@meta.data$CancerType <- factor(expt.obj@meta.data$CancerType, levels = unique(expt.obj$CancerType))
expt.obj@meta.data$TissueType <- factor(expt.obj@meta.data$TissueType, levels = c("Healthy donor", "Primary tumor tissue", "Metastatic tumor tissue", "Uninvolved normal tissue"))
...
umap_cancertype <- DimPlot(expt.obj, reduction = "umap", group.by = "CancerType", shuffle = TRUE, seed = 123)
generate_figs(umap_cancertype, paste('./plots/', experiment, '_prepare_umap_cancertype', sep = ''), c(6.5, 5))
...
barplot_cancertype_tissuetype <- BarPlotStackSplit(expt.obj, 'CancerType', 'TissueType')
generate_figs(barplot_cancertype_tissuetype, paste('./plots/', experiment, '_prepare_barplot_cancertype_tissuetype', sep = ''), c(8,4))

barplot_tissuetype_cancertype <- BarPlotStackSplit(expt.obj, 'TissueType', 'CancerType')
generate_figs(barplot_tissuetype_cancertype, paste('./plots/', experiment, '_prepare_barplot_tissuetype_cancertype', sep = ''), c(8,4))
...
barplot_cancertype_seurat_clusters <- BarPlotStackSplit(expt.obj, 'CancerType', 'seurat_clusters')
generate_figs(barplot_cancertype_seurat_clusters, paste('./plots/', experiment, '_prepare_barplot_cancertype_seurat_clusters', sep = ''), c(8,4))

barplot_tissuetype_seurat_clusters <- BarPlotStackSplit(expt.obj, 'TissueType', 'seurat_clusters')
generate_figs(barplot_tissuetype_seurat_clusters, paste('./plots/', experiment, '_prepare_barplot_tissuetype_seurat_clusters', sep = ''), c(8,4))
