...
### Script name: GSE207935-2-annotate.R
### Description: annotate seurat object for GSE207935
### Author: Vandon Duong
...
# md <- setDT(expt.obj@meta.data %>% as.data.frame)
...
barplot_azimuth_stim <- BarPlotStackSplit(expt.obj, 'azimuth', 'Stim')
generate_figs(barplot_azimuth_stim, paste('./plots/', experiment, '_prepare_barplot_azimuth_stim', sep = ''), c(8,4))

barplot_monaco_stim <- BarPlotStackSplit(expt.obj, 'monaco', 'Stim', color_set = c('deepskyblue', 'seagreen', 'darkgoldenrod', 'plum3'))
generate_figs(barplot_monaco_stim, paste('./plots/', experiment, '_prepare_barplot_monaco_stim', sep = ''), c(6,4))

barplot_monaco_affstat <- BarPlotStackSplit(expt.obj, 'monaco', 'Affstat', color_set = c('deepskyblue', 'seagreen', 'darkgoldenrod', 'plum3'))
generate_figs(barplot_monaco_affstat, paste('./plots/', experiment, '_prepare_barplot_monaco_affstat', sep = ''), c(6,4))

barplot_dice_stim <- BarPlotStackSplit(expt.obj, 'dice', 'Stim')
generate_figs(barplot_dice_stim, paste('./plots/', experiment, '_prepare_barplot_dice_stim', sep = ''), c(8,4))

barplot_phase_stim <- BarPlotStackSplit(expt.obj, 'Phase', 'Stim', color_set = hcl.colors(3, palette = "Temps"))
generate_figs(barplot_phase_stim, paste('./plots/', experiment, '_prepare_barplot_phase_stim', sep = ''), c(5,4))

barplot_phase_affstat <- BarPlotStackSplit(expt.obj, 'Phase', 'Affstat', color_set = hcl.colors(3, palette = "Temps"))
...
barplot_tex_linear_diff_affstatstim <- BarPlotStackSplit(expt.obj, 'Tex_linear_diff', 'AffstatStim', color_set =  c('royalblue', 'maroon', 'lightgrey')) + scale_x_discrete(labels = custom_labels)
barplot_tex_linear_diff_affstatstim_slim <- barplot_tex_linear_diff_affstatstim + theme(legend.position = "none")
generate_figs(barplot_tex_linear_diff_affstatstim_slim, paste('./plots/', experiment, '_prepare_barplot_tex_linear_diff_affstatstim_slim', sep = ''), c(1.75,1.75))
...
dmap_TSR <- FeaturePlot(expt.obj, reduction = 'dm', features = c("TSR"), order = FALSE, pt.size = 0.1) + fix.sc + theme(plot.title = element_blank()) + xlim(c(-0.025, 0.025)) + ylim(c(-0.025, 0.025))
dmap_Tex_Term <- FeaturePlot(expt.obj, reduction = 'dm', features = c("Tex_Term"), order = FALSE, pt.size = 0.1) + fix.sc + theme(plot.title = element_blank()) + xlim(c(-0.025, 0.025)) + ylim(c(-0.025, 0.025))
dmap_Tex_KLR <- FeaturePlot(expt.obj, reduction = 'dm', features = c("Tex_KLR"), order = FALSE, pt.size = 0.1) + fix.sc + theme(plot.title = element_blank()) + xlim(c(-0.025, 0.025)) + ylim(c(-0.025, 0.025))
